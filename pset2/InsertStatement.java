...
public class InsertStatement extends SQLStatement {
    ...
    public void execute() throws DatabaseException, DeadlockException {
        try {
            ...
            // try to insert the row to the table
            Database db = table.getDB();
            OperationStatus ret = null;
            Column primary_key = table.primaryKeyColumn();

            // primary keys cannot be overwritten
            if (primary_key != null) {
                try {
                    ret = db.putNoOverwrite(null, key, data);
                } catch (DatabaseException de) {
                    System.err.println("Error occurred while storing error");
                    return;
                }

                if (ret == OperationStatus.KEYEXIST) {
                    // System.err.println("Key already exists.");
                    throw new Exception("There is an existing row with the " +
